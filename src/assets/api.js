...
const host = 'https://xcx.gdmeika.com/'
...
export default {
   ...
   * 修改加盟商
   * @param {String} id 
   * @param {Object} data {name, app_id, secret, template_id}
   * @param {Function} cb 回调
   */
  editAppUser(id, data, cb) {
    axios.post(`${host}modify/app/${id}`, data)
      .then(res => {
        if ('200' === res.data.code) {
          typeof cb === 'function' && cb(res)
        } else {
  ...
  /**
   * 导出
   * @param {Object} data {table, start, end, state}
   * @param {Function} cb 
   */
  postExport(data, cb) {
    axios.post(`${host}output`, data)
      .then(res => {
        if ('200' === res.data.code) {
          typeof cb === 'function' && cb(res)
        } else {
